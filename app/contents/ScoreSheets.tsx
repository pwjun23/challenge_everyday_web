...
const ScoreSheets = (props : ScoreSheetProp) => {
  const {checkLists} = props;
  const {tasks_template, users_to_check} = checkLists || {};
  const [selectedDate, setSelectedDate] = useState<string>('');
  const [isEdit, setIsEdit] = useState<boolean>(false);
  const [tasks, setTasks ] = useState<{[k:string]:any}[]>([]);
  
  const onChangeTaskName =(e:ChangeEvent)=>{
    console.log({e});
  }
  useEffect(()=>{
    if(tasks_template){
    ...
    <div className="p-4 pb-20">
        {
           tasks_template && selectedDate !== '' && Object.keys(tasks_template?.[selectedDate]).sort((a:any,b:any)=> a<b?-1:a>b?1:0).map((user_id,j)=>{
            ...
            //    setTasks(tasks_template[selectedDate][user_id]);
                return (
                <div key={`score_sheet-${user_id}-${j}`}>
                    ...
                    {tasks.map((task:any, k:any)=>{
                        return(<tr key={`task-${user_id}-${k}`} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                     <td className="px-6 py-4"><input className='placeholder:text-gray-500 placeholder:italic' type='text' placeholder="Search for anything..." onChange={(e)=>onChangeTaskName(e)} value={task.task_name}/></td>
                                     <td className="px-6 py-4">{task.task_point}</td>
                                     <td className="px-6 py-4">
                                     </td>
                                 </tr>)
